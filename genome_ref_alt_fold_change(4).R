...
fit <- glmnet(as.matrix(check.na[,2:thresh]), as.numeric(check.na[,1]), alpha = 1)
summary.fit=summary(fit)
summary.fit
plot(fit) #path of its coefficient against the ℓ1-norm of the whole coefficient vector at as λ varies
print(fit) # shows us the number of non zero coeeficients for each value of lamda
coef(fit, s=0.002206) #gives us the the actual coefficients at a specific lambda


cvfit=cv.glmnet(as.matrix(check.na[,2:thresh]), as.numeric(check.na[,1]), alpha = 1)
cvfit
plot(cvfit)  #lambda and mean square error
cvfit$lambda.min #lambda.min is the value of λ that gives minimum mean cross-validated error.
coef(cvfit, s="lambda.min") 
coef(cvfit, s="lambda.1se")##gives us the the actual coefficients at a min lambda
coef_lambda.min=as.matrix(coef(cvfit, s="lambda.min")) #if we prefer non sparse format, we can pipe the output through as.matrix)
coef_lambda.1se=as.matrix(coef(cvfit, s="lambda.1se"))
coef_lambda.min
coef_lambda.1se
coef(fit, s=0.002206)
a = predict(fit, newx = as.matrix(check.na[,2:thresh]), s = 0.002206 )
a
a*check.na[,1] #checking the direction of c
