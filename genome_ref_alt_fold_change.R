...
count_fold_change=count_fold_change[order(count_fold_change$Transcript_name),]
...
for (i in 1:nmax) {
  if (refFlat$V4[i] =="+")  {
    TSSmax[i]=refFlat$V5[i] + 250000
    TSSmin[i]=refFlat$V5[i] - 250000
  } else{
    TSSmax[i]=refFlat$V6[i] + 250000
    TSSmin[i]=refFlat$V6[i] - 250000
  }
}
...
refFlat.new_nodup_c=merge(refFlat.new_nodup_b, count_norm, by="Transcript_ID", all.st_count_d=all)
#15700 141
